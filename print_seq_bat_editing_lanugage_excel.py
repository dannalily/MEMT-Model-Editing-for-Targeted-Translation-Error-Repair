...
def load_pre_edit_scores(
    ...
    if test_type == 'reliability':
        ...
        for tl in langs:
            scores = [
                re_language_score_dict[str(item['id'])][tl] 
        ...
        for tl in langs:
            ...
            scores = [
                ge_language_score_dict[str(item['id'])][i] 
                for item in editing_samples
                for i in range(3)
            ]
            before_editing[tl] = scores
        ...
        for tl in langs:
            ...
            scores = [
                lo_language_score_dict[str(item['id'])][i] 
                for item in editing_samples
                for i in range(3)
            ]
            before_editing[tl] = scores
        ...
        for tl in langs:
            scores = read_json_file(f'language_check_results/flores_{translator}_{slang}2{tl}.json')[:100]
            before_editing[tl] = scores
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        default=['reliability', 'locality', 'generality', 'flores'],
    ...
    for test_type in args.test_types:
        ...
        for step in args.steps:
            ...
            try:
                ...
                # Save results
                output_file = (
                    f'{args.output_dir}/{args.translator}/{args.slang}2{args.tlang}/language_{test_type}_step{step}_avg.xlsx'
                )
