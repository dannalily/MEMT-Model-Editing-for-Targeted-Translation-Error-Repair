...
from editors.WISE.utils import parent_module, brackets_to_periods, EarlyStopMeter, EditingMeanAct
import transformers
import numpy as np
from torch import Tensor
from torch.nn import CrossEntropyLoss
from transformers.activations import ACT2FN
from editors.WISE.merge.slerp import slerp
from editors.WISE.merge.gta import GTA
from editors.WISE.merge.linear import linear
...
def euc(query, key, config, act_mask=None, infer=False):
    """
    query/key: [B, T, H] 形状的一致张量
    返回: [B]，即每个样本一个距离（在序列维 T 上做平均；如给了 act_mask，则做掩码平均）
    """
    act_fn = ACT2FN[config.hidden_act]
    # [B, T, H] → [B, T]
    l2_norm = torch.norm(act_fn(key) - act_fn(query), dim=-1)
    ...
    if act_mask is not None:
        # act_mask 形状应为 [B, T]（bool 或 float）
        num = torch.sum(l2_norm * act_mask, dim=1)
        den = torch.sum(act_mask, dim=1).clamp_min(1e-12)
        return num / den  # [B]
    else:
...
class WISEAdapter(torch.nn.Module):
    ...
    def forward(self, *args):
        if self.editing:
            ...
            if not self.config.retrieve:
                # 普通模式：在 original / merged(self.layer) / new_weight 之间逐样本选择
                original_layer_output = self.original_layer(*args)         # [B, T, H]
                layer_output = self.layer(*args)                           # [B, T, H] (merged 权重)
                new_weight_layer_output = self.new_weight_forward(*args)   # [B, T, H]

                d_new = euc(original_layer_output, new_weight_layer_output, self.config, infer=True)  # [B]
                d_merged = euc(original_layer_output, layer_output, self.config, infer=True)          # [B]
                thr = self.editing_mean_act.min_act() * self.config.act_ratio                         # scalar

                # 逐样本掩码
                mask_orig   = (d_merged < thr) & (d_new < thr)
                mask_new    = (~mask_orig) & (d_new <= d_merged)
                mask_merged = (~mask_orig) & (~mask_new)

                layer_out = torch.empty_like(layer_output)
                if mask_orig.any():
                    layer_out[mask_orig] = original_layer_output[mask_orig]
                if mask_new.any():
                    layer_out[mask_new] = new_weight_layer_output[mask_new]
                if mask_merged.any():
                    layer_out[mask_merged] = layer_output[mask_merged]
                return layer_out

            else:
                # retrieve 模式：从 original/new_weight/memory_weight[i] 里逐样本检索最匹配的输出
                original_layer_output = self.original_layer(*args)         # [B, T, H]
                new_weight_layer_output = self.new_weight_forward(*args)   # [B, T, H]
                ...
                # 初始化逐样本输出与当前“最佳距离”
                # 若 d_new < thr → 认为不命中编辑域，用 original；否则用 new_weight
                use_original = (d_new < thr)  # [B]
                layer_out = torch.where(
                    use_original.view(-1, 1, 1),
                    original_layer_output,
                    new_weight_layer_output
                )
                min_dist = torch.where(use_original, d_new, d_new)  # 当前“匹配分数”，先用 d_new 初始化（可按需改成 d_orig）

                # 遍历 memory 权重，逐样本比较并选择更优者
                for i in range(len(self.memory_weight)):
                    memory_retrieve_weight = self.memory_weight[i]              # [H_out, H_in]
                    memory_weight_layer_output = F.linear(*args, memory_retrieve_weight)  # [B, T, H]
                    d_mem = euc(original_layer_output, memory_weight_layer_output, self.config, infer=True)  # [B]
                    mem_thr = self.memory_mean_act[i].min_act() * self.config.act_ratio  # scalar

                    # 哪些样本应当切换到该 memory：距离更大（表示更偏离 original，命中编辑域）且超过门限
                    mask_switch = (d_mem > min_dist) & (d_mem > mem_thr)  # [B]
                    if mask_switch.any():
                        layer_out[mask_switch] = memory_weight_layer_output[mask_switch]
                        min_dist[mask_switch] = d_mem[mask_switch]
