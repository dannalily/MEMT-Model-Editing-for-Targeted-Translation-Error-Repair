...
import numpy as np
from editors.ALPHAEDIT.compute_z import compute_z, get_module_input_output_at_words
from editors.ALPHAEDIT.compute_ks import compute_ks
from util_editing.generate import input_format
from util_editing.layer_stats import layer_stats
import os
...
def apply_AlphaEdit_to_model(
    ...
    **kwargs
    ...
    deltas = execute_AlphaEdit(model, tok, requests, hparams,  system_prompt, random_context_prompt, kl_prompt, cache_template=cache_template,cache_accu=cache_accu)
...
def execute_AlphaEdit(
    ...
    context_templates = get_context_templates(model, tok, hparams.model_name, random_context_prompt)
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        layer_ks = compute_ks(model, tok, requests, hparams, layer, context_templates,system_prompt).T
        ...
        cur_zs = get_module_input_output_at_words(
            ...
            tok,
            z_layer,
            context_templates=input_format(
                tok,[request["prompt"] for request in requests],system_prompt,chat_temp = True if hparams.model_name in ["Qwen/Qwen2.5-3B-Instruct","Qwen/Qwen2.5-7B-Instruct"] else False),
...
def get_context_templates(model, tok, model_name, random_context_prompt):
