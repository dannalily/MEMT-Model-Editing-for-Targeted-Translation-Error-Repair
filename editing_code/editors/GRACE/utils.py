...
def tokenize(batch, tokenizer, device, system_prompt, hparams, test=False):
    ...
    if test or not label:
        if hasattr(hparams, 'use_chat_template') and hparams.use_chat_template:
            tokens = tokenizer([tokenizer.apply_chat_template([{"role": "system", "content": system_prompt},{"role":"user", "content":p}],
                                    add_generation_prompt=True,
                                    tokenize=False) for p in prompt], return_tensors="pt", padding=True, truncation=True)
        else:
        ...
        if hasattr(hparams, 'use_chat_template') and hparams.use_chat_template:
            full_prompt = [tokenizer.apply_chat_template([{"role": "system", "content": system_prompt},{"role":"user", "content":p}],
                                        add_generation_prompt=True,
                                        tokenize=False) + ' ' + l
                                        for p, l in zip(prompt, label)]
            prompt_ids = tokenizer([tokenizer.apply_chat_template([{"role": "system", "content": system_prompt},{"role":"user", "content":p}],
                                    add_generation_prompt=True,
                                    tokenize=False) 
                                    for p in prompt], return_tensors="pt", padding=True, truncation=True)["input_ids"]
        else:
