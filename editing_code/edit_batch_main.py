...
def generate_response(model, tok, requests, question_type, hparams, all_tlangs=[]):
    ...
    def qwen_query_batch(system_prompts, user_prompts, max_new_tokens=200, num_beams=5, do_sample=False, temperature=None, top_p=None, top_k=None):
        ...
        del encoded_inputs, generated_ids, inputs, all_messages
...
def main():
    ...
    set_seed(42)
    test_batch_sizes = [1,4,16,32,64,128,256]
    ...
    # editing_samples = random.sample(editing_samples, max(test_batch_sizes))
    editing_samples = random.sample(editing_samples, 1024)
    # random.shuffle(editing_samples)
    ids= [sam['id'] for sam in editing_samples]
    save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/{args.slang}_{args.tlang}-seed{args.seed}.json'
    ...
    for size in test_batch_sizes:
        hparams.batch_size =size
        random_elements=[]
        if hparams.alg_name in ['UNKE']:
            random_elements = random.sample(ex_datas, 20)

        edited_model, weights_copy = apply_algo(model, tok, requests[:size], hparams, copy=False,return_orig_weights=True, system_prompt=system_prompt, random_context_prompt=random_context_prompt, kl_prompt=kl_prompt,cache_accu=False,ex_data=random_elements) 
        ## set copy=False to save memory
 
        print(f"batch_size: {size}")
        
        print("evaluate: reliability")
        r_responses = generate_response(edited_model, tok, requests[:size], 'reliability', hparams, all_tlangs)
        
        print("evaluate: generality")
        g_responses = generate_response(edited_model, tok, requests[:size], 'generality', hparams, all_tlangs)

        print("evaluate: locality-memt")
        l_responses = generate_response(edited_model, tok, requests[:size], 'locality', hparams, all_tlangs)
        ...
        save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}_layer{layers_str}_r-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        write_json_file(r_responses, save_file)
        save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}_layer{layers_str}_g-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        write_json_file(g_responses, save_file)
        save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}_layer{layers_str}_l-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        write_json_file(l_responses, save_file)
        save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}_layer{layers_str}_m-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        write_json_file(m_responses, save_file)
        save_file = f'editing_batch_eval/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}/mmlu_layer{layers_str}-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
        write_json_file(m_eval, save_file)
        save_file = f'editing_batch_results/{model_suffix}/{args.editing_method}/batch{size}/{args.slang}_{args.tlang}_layer{layers_str}_f-seed{args.seed}.json'
        os.makedirs(os.path.dirname(save_file), exist_ok=True)
