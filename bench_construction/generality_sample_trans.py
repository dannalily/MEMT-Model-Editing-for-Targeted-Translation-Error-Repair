"""
Module for translating generality sample sentences using batch inference.

This script translates generality sample sentences from source languages
...
INPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/generality_{lang}_sent.json'
OUTPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/generality_{slang}_{tlang}_trans.json'
BATCH_INPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/batch_cache/generality_{slang}_{tlang}_trans_batch{batch_num}.jsonl'
BATCH_OUTPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/batch_cache/generality_{slang}_{tlang}_trans_out_batch{batch_num}.jsonl'


# =========================
# Helper functions
# =========================
def get_source_key(source_lang: str) -> str:
    """
    Get the appropriate source key based on source language.
    
    Args:
        source_lang: Source language code
        
    Returns:
        Key name for extracting source text
    """
    return 'Plain_English' if source_lang == 'en' else 'English'
...
def prepare_prompts(
    ...
    system_prompts = []
    user_prompts = []
    source_key = get_source_key(source_lang)
...
def process_language_pair(
    ...
    try:
        ...
        system_prompts, user_prompts = prepare_prompts(data, source_lang, target_lang)
...
def main() -> None:
    ...
    try:
        logger.info("Starting generality sentence translation")
