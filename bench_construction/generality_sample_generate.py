"""
Module for generating generality sample sentences for idioms using batch inference.

This script processes idiom data and generates multiple sample sentences
...
S3_BUCKET = 'dann02'
S3_INPUT_DIR = 'generality_sample/input'
S3_OUTPUT_DIR = 'generality_sample/output'
...
BATCH_INPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/batch_cache/generality_{lang}_sent.jsonl'
BATCH_OUTPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/batch_cache/generality_{lang}_sent_out.jsonl'
FINAL_OUTPUT_PATH_TEMPLATE = 'bench_construction/intermediate_out/generality_{lang}_sent.json'
...
def build_user_prompt(
    ...
    para_item: Dict[str, Any]
) -> str:
    """
    Build user prompt for generality sentence generation.
    
    Args:
        source_lang: Source language code ('en' or 'zh')
        idiom_data: Idiom knowledge base item
        para_item: Parallel idiom sentence item
        
    Returns:
        Formatted user prompt string
    """
    if source_lang == 'zh':
        return PROMPT_DICT["GENERALITY_ZH_EN"]["user"].format(
            Idiom=idiom_data['idiom'],
            ZH_Meaning=idiom_data['zh_meaning'],
            EN_Meaning=idiom_data['en_meaning'],
            Idiom_sent=para_item['src'],
            Plain_Idiom_sent=para_item['src_plain'],
            Translation=para_item['tgt']['English']
        )
    else:  # en
        return PROMPT_DICT["GENERALITY_EN_EN"]["user"].format(
            Idiom=idiom_data['idiom'],
            EN_Meaning=idiom_data['en_meaning'],
            Idiom_sent=para_item['src'],
            Plain_Idiom_sent=para_item['src_plain']
...
def prepare_prompts(
    ...
    prompt_key = f"GENERALITY_{source_lang.upper()}_EN"
    ...
    for item in para_data:
        ...
        user_prompt = build_user_prompt(source_lang, idiom_data, item)
...
def process_language(source_lang: str) -> None:
    """
    Process generality sentence generation for a specific source language.
...
def main() -> None:
    """Main function to process generality sentence generation for all languages."""
    try:
        logger.info("Starting generality sentence generation")
