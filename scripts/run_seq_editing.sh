...
mkdir -p logs_seq_editing logs_seq_editing_eval logs_script
...
# Master script log
SCRIPT_LOG="logs_script/script_seq_editing.log"
...
for slang in "${slangs[@]}"; do
    for tlang in "${tlangs[@]}"; do
        ...
        for method in "${methods[@]}"; do
            # Array to store PIDs
            pids_editing=()
            mkdir -p logs_seq_editing/${method,,}/qwen3b_${slang}_${tlang}

            for i in "${!seeds[@]}"; do
                seed=${seeds[$i]}
                gpu=${gpus[$i]}
            
                echo "Starting sequential editing: $method with seed $seed on GPU $gpu "
                
                CUDA_VISIBLE_DEVICES=$gpu python -u editing_code/edit_sequential_main.py \
                    --editing_method $method \
                    --hparam_file editing_code/hparams/$method/qwen2-5-3b-instruct_${slang}2x.yaml \
                    --slang $slang \
                    --tlang $tlang \
                    --seed $seed \
                    > logs_seq_editing/${method,,}/qwen3b_${slang}_${tlang}/${seed}.txt 2>&1 &
                pids_editing+=($!)
                echo "  → PID: $! (${method})"
            done
        ...
        for method in "${methods[@]}"; do    
            ...
            mkdir -p logs_seq_editing_eval/${method,,}/qwen3b_${slang}_${tlang}
            pids_eval=()
            ...
            for i in "${!seeds[@]}"; do
                ...
                echo "Starting sequential editing evaluation: $method with seed $seed on GPU $gpu"
                ...
                CUDA_VISIBLE_DEVICES=$gpu python -u evaluation_editing.py \
                    ...
                    --type sequential \
                    --seed $seed \
                    > logs_seq_editing_eval/${method,,}/qwen3b_${slang}_${tlang}/${seed}.txt 2>&1 &
        ...
        python language_detection.py \
                --editing_type sequential \
                --editing_slang $slang \
                --editing_tlang $tlang \
                > logs_seq_editing_eval/language_${slang}_${tlang}.txt 2>&1
        ...
        python print_seq_bat_editing_eval_results_excel.py \
            ...
            --output-dir excel_seq_editing \
            > logs_seq_editing_eval/excel_editing_${slang}_${tlang}.txt 2>&1
        ...
        python print_seq_bat_editing_eval_results_plot.py \
            ...
            --tlang $tlang \
            --editing-type sequential \
             --input_dir excel_seq_editing \
             --output_dir plot_seq_editing \
            > logs_seq_editing_eval/plot_editing_${slang}_${tlang}.txt 2>&1
        ...
        python print_seq_bat_editing_lanugage_excel.py \
            ...
            --tlang $tlang \
            --editing-type sequential \
            --output-dir excel_seq_language \
            > logs_seq_editing_eval/excel_language_${slang}_${tlang}.txt 2>&1
        ...
        python print_seq_bat_editing_eval_results_plot.py \
            ...
            --metric language \
            --editing-type sequential \
             --input_dir excel_seq_language \
             --output_dir plot_seq_language \
            > logs_seq_editing_eval/plot_language_${slang}_${tlang}.txt 2>&1
